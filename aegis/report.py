...
def to_markdown(report: Dict[str, Any]) -> str:
    ...
    for item in report.get("findings", []):
        ...
        if risk_class:
            posture[risk_class] = posture.get(risk_class, 0) + 1
    posture_line = ""
    if posture:
        posture_line = "**AI risk posture:** %s  " % ", ".join(
            "`%s` ×%d" % (name, count) for name, count in sorted(posture.items())
        )
    lines = [
        title,
        "",
