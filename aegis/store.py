...
class TaskStore:
    ...
    def _init(self) -> None:
        with self._connect() as conn:
            ...
            # Content-level trajectory traces for the flywheel (self-optimization
            # data pipeline). SQLite-only by design, mirroring release_observations:
            # the pipeline runs on the local task store, not the Postgres mirror.
            conn.execute(
            ...
            conn.execute(
                """CREATE INDEX IF NOT EXISTS idx_llm_traces_task ON llm_traces(task_id, id)"""
            )
            conn.execute(
            ...
            conn.execute(
                """CREATE INDEX IF NOT EXISTS idx_task_labels_task ON task_labels(task_id, id)"""
            )
            conn.execute(
            ...
            conn.execute(
                """CREATE INDEX IF NOT EXISTS idx_sft_samples_kind ON sft_samples(kind, status)"""
            )
            conn.execute(
            ...
            conn.execute(
                """CREATE TABLE IF NOT EXISTS model_adapters (
                    ...
                    created_at TEXT NOT NULL,
                    UNIQUE(skill_name, layer, version)
                )"""
            )
            conn.execute(
                """CREATE INDEX IF NOT EXISTS idx_model_adapters_active
                   ON model_adapters(layer, active)"""
            )
            conn.execute(
                """CREATE TABLE IF NOT EXISTS experience_suggestions (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    task_id TEXT,
                    repository TEXT,
                    rule_id TEXT,
            ...
            conn.execute(
                """CREATE INDEX IF NOT EXISTS idx_experience_status
                   ON experience_suggestions(status, repository)"""
            )
            # Hermes-style cross-session review history search (SQLite FTS5).
            self._fts_available = True
            try:
                conn.execute(
                    """CREATE VIRTUAL TABLE IF NOT EXISTS review_fts USING fts5(
                        tenant_id, repository, task_id, content, created_at
                    )"""
                )
    ...
    # --- flywheel: content-level trajectory traces -----------------------------
    def save_llm_trace(self, record: Dict[str, Any]) -> int:
        created_at = record.get("created_at") or utc_now()
        with self._lock, self._connect() as conn:
            cursor = conn.execute(
                "INSERT INTO llm_traces(task_id, role, step, event_type, model, provider, "
                "prompt_json, reply_json, tool_name, tool_args_json, tool_ok, "
                "result_preview, created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (
                    str(record.get("task_id", ""))[:64],
                    str(record.get("role", "unknown"))[:64],
                    int(record.get("step", 0)),
                    str(record.get("event_type", "llm"))[:16],
                    str(record.get("model") or "")[:200] or None,
                    str(record.get("provider") or "")[:64] or None,
                    self._dumps(record.get("prompt")),
                    self._dumps(record.get("reply")),
                    str(record.get("tool_name") or "")[:128] or None,
                    self._dumps(record.get("tool_args")),
                    None if record.get("tool_ok") is None
                    else int(bool(record.get("tool_ok"))),
                    str(record.get("result_preview") or "")[:4000] or None,
                    created_at,
                ),
            )
            return int(cursor.lastrowid)

    def list_task_llm_traces(self, task_id: str, limit: int = 2000) -> list:
        with self._connect() as conn:
            rows = conn.execute(
                "SELECT * FROM llm_traces WHERE task_id=? ORDER BY id LIMIT ?",
                (task_id, max(1, min(limit, 20000))),
            ).fetchall()
        return [self._llm_trace_from_row(row) for row in rows]

    def list_llm_traces(self, after_id: int = 0, limit: int = 500) -> list:
        with self._connect() as conn:
            rows = conn.execute(
                "SELECT * FROM llm_traces WHERE id>? ORDER BY id LIMIT ?",
                (int(after_id), max(1, min(limit, 20000))),
            ).fetchall()
        return [self._llm_trace_from_row(row) for row in rows]

    def count_llm_traces(self) -> int:
        with self._connect() as conn:
            row = conn.execute("SELECT COUNT(*) AS n FROM llm_traces").fetchone()
        return int(row["n"])

    @staticmethod
    def _llm_trace_from_row(row) -> Dict[str, Any]:
        value = dict(row)
        value["prompt"] = (
            json.loads(value.pop("prompt_json")) if value.get("prompt_json") else None
        )
        value["reply"] = (
            json.loads(value.pop("reply_json")) if value.get("reply_json") else None
        )
        value["tool_args"] = (
            json.loads(value.pop("tool_args_json"))
            if value.get("tool_args_json") else None
        )
        return value
    ...
    # --- flywheel: scoring / annotation labels ---------------------------------
    def save_task_label(
        self, task_id: str, dimension: str, score: Optional[float] = None,
        label: str = "", scored_by: str = "human", note: str = "",
        metadata: Optional[Dict[str, Any]] = None,
    ) -> int:
        with self._lock, self._connect() as conn:
            cursor = conn.execute(
                "INSERT INTO task_labels(task_id, dimension, score, label, scored_by, "
                "note, metadata_json, created_at) VALUES (?,?,?,?,?,?,?,?)",
                (
                    str(task_id)[:64], str(dimension)[:64],
                    None if score is None else float(score),
                    str(label)[:64], str(scored_by)[:32], str(note)[:4000],
                    self._dumps(metadata), utc_now(),
                ),
            )
            return int(cursor.lastrowid)

    def list_task_labels(self, task_id: str) -> list:
        with self._connect() as conn:
            rows = conn.execute(
                "SELECT * FROM task_labels WHERE task_id=? ORDER BY id DESC", (task_id,)
            ).fetchall()
        return [self._label_from_row(row) for row in rows]

    def list_task_labels_all(
        self, scored_by: Optional[str] = None, after_id: int = 0, limit: int = 500,
    ) -> list:
        clauses = []
        params: list = []
        if scored_by:
            clauses.append("scored_by = ?")
        ...
        with self._connect() as conn:
            rows = conn.execute(
                "SELECT * FROM task_labels WHERE " + " AND ".join(clauses)
                + " ORDER BY id LIMIT ?", params,
            ).fetchall()
        return [self._label_from_row(row) for row in rows]

    @staticmethod
    def _label_from_row(row) -> Dict[str, Any]:
        value = dict(row)
        value["metadata"] = (
            json.loads(value.pop("metadata_json"))
            if value.get("metadata_json") else {}
        )
        return value

    # --- flywheel: SFT sample pool ---------------------------------------------
    def record_sft_samples(
        self, samples: list, dataset_version: str = "", status: str = "pool",
    ) -> int:
        rows = []
        created_at = utc_now()
        for sample in samples:
            rows.append((
                str(sample.get("task_id") or "")[:64],
                str(sample.get("kind") or "classify")[:32],
                status,
                self._dumps(sample.get("payload") or sample),
                self._dumps(sample.get("provenance") or {}),
                dataset_version or "",
                created_at,
            ))
        if not rows:
        ...
        with self._lock, self._connect() as conn:
            conn.executemany(
                "INSERT INTO sft_samples(task_id, kind, status, payload_json, "
                "provenance_json, dataset_version, created_at) VALUES (?,?,?,?,?,?,?)",
                rows,
            )
        return len(rows)

    def list_sft_samples(
        self, kind: Optional[str] = None, status: Optional[str] = None,
        dataset_version: Optional[str] = None, limit: int = 1000,
    ) -> list:
        clauses = []
        params: list = []
        for column, value in (
            ("kind", kind), ("status", status), ("dataset_version", dataset_version)
        ):
            if value:
        ...
        with self._connect() as conn:
            rows = conn.execute(
                "SELECT * FROM sft_samples WHERE " + (" AND ".join(clauses) if clauses else "1=1")
                + " ORDER BY id LIMIT ?", params,
            ).fetchall()
        values = []
        for row in rows:
            ...
            item["provenance"] = (
                json.loads(item.pop("provenance_json"))
                if item.get("provenance_json") else {}
            )
            values.append(item)
        return values

    def count_sft_samples(
        self, kind: Optional[str] = None, status: Optional[str] = None,
    ) -> int:
        clauses = []
        params: list = []
        ...
        with self._connect() as conn:
            row = conn.execute(
                "SELECT COUNT(*) AS n FROM sft_samples WHERE "
                + (" AND ".join(clauses) if clauses else "1=1"), params,
            ).fetchone()
        return int(row["n"])

    def purge_sft_pool(self, dataset_version: str) -> int:
        """Remove previously registered samples for a dataset version."""
        with self._lock, self._connect() as conn:
            cursor = conn.execute(
                "DELETE FROM sft_samples WHERE dataset_version=?",
                (dataset_version,),
            )
            return int(cursor.rowcount)

    # --- flywheel: training runs and adapter registry --------------------------
    def save_training_run(self, run: Dict[str, Any]) -> None:
        with self._lock, self._connect() as conn:
            conn.execute(
                "INSERT INTO training_runs(id, run_type, base_model, dataset_version, "
                "dataset_hash, hyperparams_json, metrics_json, status, adapter_id, "
                "created_at, finished_at) VALUES (?,?,?,?,?,?,?,?,?,?,?) "
                "ON CONFLICT(id) DO UPDATE SET status=excluded.status, "
                "metrics_json=excluded.metrics_json, finished_at=excluded.finished_at, "
                "adapter_id=excluded.adapter_id",
                (
                    run["id"], run.get("run_type", "sft"), run.get("base_model"),
                    run.get("dataset_version"), run.get("dataset_hash"),
                    self._dumps(run.get("hyperparams") or {}),
                    self._dumps(run.get("metrics") or {}),
                    run.get("status", "pending"), run.get("adapter_id"),
                    run.get("created_at") or utc_now(), run.get("finished_at"),
                ),
            )
    ...
    def list_training_runs(self, limit: int = 50) -> list:
        with self._connect() as conn:
            rows = conn.execute(
    ...
    @staticmethod
    def _training_run_from_row(row) -> Dict[str, Any]:
        value = dict(row)
        value["hyperparams"] = (
            json.loads(value.pop("hyperparams_json"))
            if value.get("hyperparams_json") else {}
        )
        value["metrics"] = (
            json.loads(value.pop("metrics_json"))
            if value.get("metrics_json") else {}
        )
        return value

    def save_adapter(
        self, layer: str, adapter: Dict[str, Any], score: float,
        activate: bool = False, skill_name: str = "llm-review",
    ) -> Dict[str, Any]:
        with self._lock, self._connect() as conn:
            row = conn.execute(
                "SELECT COALESCE(MAX(version),0) AS version FROM model_adapters "
                "WHERE skill_name=? AND layer=?", (skill_name, layer),
            ).fetchone()
            version = int(row["version"]) + 1
            parent = conn.execute(
                "SELECT version FROM model_adapters WHERE skill_name=? AND layer=? "
                "AND active=1 ORDER BY version DESC LIMIT 1",
                (skill_name, layer),
            ).fetchone()
            if activate:
                conn.execute(
                    "UPDATE model_adapters SET active=0 WHERE skill_name=? AND layer=?",
                    (skill_name, layer),
                )
            created_at = utc_now()
            conn.execute(
                "INSERT INTO model_adapters(skill_name, layer, version, base_model, "
                "adapter_sha256, adapter_dir, serving_model_id, score, metrics_json, "
                "active, parent_version, status, created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (
                    skill_name, layer, version, adapter.get("base_model"),
                    adapter.get("adapter_sha256"), adapter.get("adapter_dir"),
                    adapter.get("serving_model_id"), float(score),
                    self._dumps(adapter.get("metrics") or {}),
                    int(activate),
                    parent["version"] if parent else None,
                    str(adapter.get("status") or "draft"),
                    created_at,
                ),
            )
        return {
            "skill_name": skill_name, "layer": layer, "version": version,
            "score": float(score), "active": bool(activate),
            "parent_version": parent["version"] if parent else None,
            "adapter_sha256": adapter.get("adapter_sha256"),
        }

    def get_active_adapter(
        self, layer: str, skill_name: str = "llm-review",
    ) -> Optional[Dict[str, Any]]:
        with self._connect() as conn:
            row = conn.execute(
                "SELECT * FROM model_adapters WHERE skill_name=? AND layer=? AND active=1 "
                "ORDER BY version DESC LIMIT 1", (skill_name, layer),
            ).fetchone()
        return self._adapter_from_row(row) if row else None

    def list_adapters(self, layer: str, skill_name: str = "llm-review") -> list:
        with self._connect() as conn:
            rows = conn.execute(
                "SELECT * FROM model_adapters WHERE skill_name=? AND layer=? "
                "ORDER BY version DESC", (skill_name, layer),
            ).fetchall()
        return [self._adapter_from_row(row) for row in rows]

    def activate_adapter(
        self, layer: str, version: int, skill_name: str = "llm-review",
    ) -> bool:
        with self._lock, self._connect() as conn:
            exists = conn.execute(
                "SELECT 1 FROM model_adapters WHERE skill_name=? AND layer=? AND version=?",
                (skill_name, layer, version),
            ).fetchone()
            if not exists:
                return False
            conn.execute(
            ...
            conn.execute(
                "UPDATE model_adapters SET status='staging', active=1 "
                "WHERE skill_name=? AND layer=? AND version=?",
                (skill_name, layer, version),
            )
            conn.execute(
                "UPDATE model_adapters SET status='active' WHERE skill_name=? AND layer=? "
                "AND active=1", (skill_name, layer),
            )
        return True

    @staticmethod
    def _adapter_from_row(row) -> Dict[str, Any]:
        value = dict(row)
        value["metrics"] = (
            json.loads(value.pop("metrics_json"))
            if value.get("metrics_json") else {}
    ...
    def count_task_labels(self) -> int:
        with self._connect() as conn:
            row = conn.execute("SELECT COUNT(*) AS n FROM task_labels").fetchone()
        return int(row["n"])

    def count_labeled_tasks(self) -> int:
        with self._connect() as conn:
            row = conn.execute(
                "SELECT COUNT(DISTINCT task_id) AS n FROM task_labels"
            ).fetchone()
        return int(row["n"])

    # --- experience: mined suggestions for skill self-consolidation ------------
    def save_experience_suggestion(
        self, repository: str, rule_id: str, risk_class: str, title: str,
        rationale: str, payload: Dict[str, Any], task_id: str = "",
    ) -> int:
        with self._lock, self._connect() as conn:
            cursor = conn.execute(
                "INSERT INTO experience_suggestions(task_id, repository, rule_id, "
                "risk_class, title, rationale, payload_json, status, created_at) "
                "VALUES (?,?,?,?,?,?,?,?,?)",
                (str(task_id)[:64], str(repository)[:250], str(rule_id)[:80],
                 str(risk_class)[:64], str(title)[:400], str(rationale)[:2000],
                 self._dumps(payload), "pending", utc_now()),
            )
            return int(cursor.lastrowid)

    def has_experience_suggestion(
        self, repository: str, rule_id: str, task_id: str = "",
    ) -> bool:
        with self._connect() as conn:
            row = conn.execute(
                "SELECT 1 FROM experience_suggestions WHERE repository=? AND rule_id=?",
                (repository, rule_id),
            ).fetchone()
            return row is not None

    def list_experience_suggestions(
        ...
        with self._connect() as conn:
            rows = conn.execute(
                "SELECT * FROM experience_suggestions WHERE "
        ...
        for row in rows:
            item = dict(row)
            item["payload"] = (
                json.loads(item.pop("payload_json"))
                if item.get("payload_json") else {}
            )
            values.append(item)
        return values

    def get_experience_suggestion(self, suggestion_id: int) -> Optional[Dict[str, Any]]:
        with self._connect() as conn:
            row = conn.execute(
                "SELECT * FROM experience_suggestions WHERE id=?",
                (int(suggestion_id),),
            ).fetchone()
        if row is None:
            return None
    ...
    def set_experience_suggestion_status(
        self, suggestion_id: int, status: str, artifact_version: Optional[int] = None,
    ) -> bool:
        with self._lock, self._connect() as conn:
            cursor = conn.execute(
                "UPDATE experience_suggestions SET status=?, artifact_version=? "
                "WHERE id=?",
                (status, artifact_version, int(suggestion_id)),
            )
            return int(cursor.rowcount) == 1

    # --- review history search (FTS5) & repository-scoped reads -----------------
    def index_review_text(
        self, tenant_id: str, repository: str, task_id: str, content: str,
        created_at: Optional[str] = None,
    ) -> bool:
        if not getattr(self, "_fts_available", False) or not content.strip():
            return False
        created_at = created_at or utc_now()
        with self._lock, self._connect() as conn:
            conn.execute("DELETE FROM review_fts WHERE task_id=?", (task_id,))
            conn.execute(
                "INSERT INTO review_fts(tenant_id, repository, task_id, content, "
                "created_at) VALUES (?,?,?,?,?)",
                (tenant_id, repository, task_id, content[:40000], created_at),
            )
        return True

    def search_review_history(
        self, tenant_id: str, repository: str, query: str, limit: int = 20,
    ) -> list:
        if not getattr(self, "_fts_available", False):
            return []
        tokens = re.findall(
            r"[A-Za-z0-9_./:-]+|[㐀-鿿]+", str(query or "")
        )
        if not tokens:
            return []
        match = " OR ".join('"%s"' % token.replace('"', "") for token in tokens[:24])
        with self._connect() as conn:
            rows = conn.execute(
                "SELECT task_id, repository, content, created_at FROM review_fts "
                "WHERE tenant_id=? AND repository=? AND review_fts MATCH ? "
                "ORDER BY created_at DESC LIMIT ?",
                (tenant_id, repository, match, max(1, min(limit, 100))),
            ).fetchall()
        return [dict(row) for row in rows]

    def list_repository_reports(
        self, tenant_id: str, repository: str, limit: int = 300,
    ) -> list:
        with self._connect() as conn:
            rows = conn.execute(
                "SELECT id, state, report_json, created_at FROM tasks "
                "WHERE tenant_id=? AND repository=? AND report_json IS NOT NULL "
                "ORDER BY created_at DESC LIMIT ?",
                (tenant_id, repository, max(1, min(limit, 2000))),
            ).fetchall()
        values = []
        for row in rows:
            item = dict(row)
    ...
    def list_repository_failure_cases(
        self, tenant_id: str, repository: str, limit: int = 500,
    ) -> list:
        with self._connect() as conn:
            rows = conn.execute(
                "SELECT f.* FROM failure_cases f JOIN tasks t ON t.id=f.task_id "
                "WHERE t.tenant_id=? AND t.repository=? ORDER BY f.id DESC LIMIT ?",
                (tenant_id, repository, max(1, min(limit, 2000))),
            ).fetchall()
        values = []
        for row in rows:
    ...
    # --- governance periodic reports -------------------------------------------
    def save_governance_report(
        self, report_id: str, repository: str, since: str, report: Dict[str, Any],
    ) -> None:
        with self._lock, self._connect() as conn:
            conn.execute(
                "INSERT OR REPLACE INTO governance_reports(id, repository, since, "
                "report_json, created_at) VALUES (?,?,?,?,?)",
                (report_id, repository, since,
                 json.dumps(report, ensure_ascii=False), utc_now()),
            )

    def list_governance_reports(self, repository: str = "", limit: int = 20) -> list:
        clauses = []
        params: list = []
        if repository:
        ...
        with self._connect() as conn:
            rows = conn.execute(
                "SELECT id, repository, since, report_json, created_at FROM governance_reports WHERE "
                + (" AND ".join(clauses) if clauses else "1=1")
