...
TASK_LABELS = re.compile(r"^/v1/tasks/([A-Za-z0-9_-]+)/labels$")
ADAPTER_LIST = re.compile(r"^/v1/adapters/([A-Za-z0-9_-]+)$")
ADAPTER_ACTIVATE = re.compile(
    r"^/v1/adapters/([A-Za-z0-9_-]+)/versions/(\d+)/activate$"
)
EXPERIENCE_SUGGESTIONS = re.compile(r"^/v1/experience/suggestions$")
EXPERIENCE_ACTION = re.compile(
    r"^/v1/experience/suggestions/(\d+)/(accept|dismiss)$"
)
...
class ApiHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path == "/assets/app.js":
            ...
            return
        if path == "/assets/app.theme.css":
            self._serve_file("app.theme.css")
            return
        if path == "/health":
            mode = resolve_mode(
        ...
        if match:
            ...
            return
        if path == "/v1/flywheel/status":
            self._send_json(200, self.service.flywheel_status())
            return
        if path == "/v1/flywheel/replay":
            self._send_json(200, self.service.flywheel_replay())
            return
        if path == "/v1/memory/search":
            self._send_json(200, self.service.history_search(
                query.get("repository", [""])[0], query.get("q", [""])[0],
                principal.tenant_id,
            ))
            return
        if path == "/v1/memory/digest":
            self._send_json(200, self.service.history_digest(
                query.get("repository", [""])[0], principal.tenant_id,
            ))
            return
        if path == "/v1/governance/reports":
            self._send_json(200, self.service.list_governance_reports(
                query.get("repository", [""])[0],
                int(query.get("limit", [20])[0]),
            ))
            return
        if path == "/v1/flywheel/training-runs":
            self._send_json(200, {"runs": self.service.store.list_training_runs(
                int(query.get("limit", [20])[0])
            )})
            return
        if path == "/v1/flywheel/datasets":
            self._send_json(200, {"pool": {
                kind: self.service.store.count_sft_samples(kind=kind, status="selected")
                for kind in SAMPLE_KINDS
            }})
            return
        match = ADAPTER_LIST.match(path)
        if match:
            layer = match.group(1)
            self._send_json(200, {
                "layer": layer,
                "adapters": self.service.store.list_adapters(layer),
            })
            return
        match = EXPERIENCE_SUGGESTIONS.match(path)
        if match:
            self._send_json(200, self.service.list_experience_suggestions(
                query.get("status", [""])[0]
            ))
            return
        match = TASK_LABELS.match(path)
        if match:
            if not principal.can("review"):
                self._send_json(403, {"error": "permission denied"})
                return
            task = self.service.store.get(match.group(1), principal.tenant_id)
            if not task:
                self._send_json(404, {"error": "task not found"})
                return
            self._send_json(200, {
                "labels": self.service.store.list_task_labels(match.group(1))
            })
            return
        if path == "/github/install":
            if not self.settings.github_app_slug:
                self._send_json(503, {"error": "AEGIS_GITHUB_APP_SLUG is not configured"})
                return
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if match:
                principal = self._principal("review")
                payload = self._read_json(body)
                result = self.service.add_task_label(
                    match.group(1),
                    str(payload.get("dimension", "")),
                    payload.get("score"),
                    str(payload.get("label", "")),
                    str(payload.get("note", "")),
                    str(payload.get("scored_by", "human")),
                    payload.get("metadata"),
                    principal.tenant_id,
                )
                self.service.store.audit(
                    principal.tenant_id, principal.username, "task.label",
                    match.group(1), {"dimension": result["dimension"]},
                )
                self._send_json(201, result)
                return
            if path == "/v1/flywheel/pipeline/run":
                principal = self._principal("manage")
                payload = self._read_json(body)
                manifest = self.service.flywheel_pipeline_run(payload.get("options"))
                self.service.store.audit(
                    principal.tenant_id, principal.username, "flywheel.pipeline",
                    "dataset", {"dataset_version": manifest.get("dataset_version")},
                )
                self._send_json(201, manifest)
                return
            if path == "/v1/flywheel/train":
                principal = self._principal("manage")
                payload = self._read_json(body)
                result = self.service.flywheel_train_run(payload.get("options"))
                self.service.store.audit(
                    principal.tenant_id, principal.username, "flywheel.train",
                    result.get("layer", ""), {"run_id": result.get("run_id")},
                )
                self._send_json(201, result)
                return
            ...
            if match:
                ...
                self._send_json(200 if ok else 404, {"activated": ok})
                return
            match = ADAPTER_ACTIVATE.match(path)
            if match:
                principal = self._principal("manage")
                ok = self.service.activate_adapter(match.group(1), int(match.group(2)))
                self.service.store.audit(
                    principal.tenant_id, principal.username, "adapter.activate",
                    match.group(1), {"version": int(match.group(2)), "activated": ok},
                )
                self._send_json(200 if ok else 404, {"activated": ok})
                return
            match = EXPERIENCE_ACTION.match(path)
            if match:
                principal = self._principal("manage")
                suggestion_id = int(match.group(1))
                action = match.group(2)
                result = (
                    self.service.accept_experience_suggestion(
                        suggestion_id, principal.tenant_id,
                    )
                    if action == "accept"
                    else self.service.dismiss_experience_suggestion(
                        suggestion_id, principal.tenant_id,
                    )
                )
                self._send_json(200, result)
            ...
            if path == "/v1/governance/report":
                principal = self._principal("manage")
                payload = self._read_json(body)
                result = self.service.run_governance_report(
                    str(payload.get("repository", "")),
                    str(payload.get("since", "")),
                    principal.tenant_id,
                )
                self.service.store.audit(
                    principal.tenant_id, principal.username, "governance.report",
...
def run() -> None:
    ...
    print("AegisAgent dashboard: http://%s:%d" % (settings.host, settings.port))
