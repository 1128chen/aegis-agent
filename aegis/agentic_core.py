...
from .models import ComponentKind, Finding, Severity, normalize_risk_class
...
class BoundedRole:
    def __init__(
        ...
        # Optional sink for the flywheel trajectory capture. Receives one record
        # per model step (and one per tool call); failures never break the loop.
    ...
    def run(
        ...
        for step in range(1, self.max_steps + 1):
            ...
            managed_user = json.dumps(managed, ensure_ascii=False, default=str)
            ...
            self._emit_trace({
                "event_type": "llm", "role": self.name, "step": step,
                "model": getattr(self.client, "model", None),
                "provider": getattr(self.client, "provider", None),
                "prompt": {"system": self.prompt, "user": managed_user},
                "reply": {
            ...
            self._emit_trace({
                "event_type": "tool", "role": self.name, "step": step,
                "model": getattr(self.client, "model", None),
                "provider": getattr(self.client, "provider", None),
                "tool_name": tool_name, "tool_args": arguments,
                "tool_ok": observation.get("ok", False),
                "result_preview": (
                    json.dumps(
                        observation.get("result"), ensure_ascii=False, default=str
                    )[:2000]
                    if observation.get("ok")
                    else str(observation.get("error", ""))[:1000]
                ),
            })
...
def _parse_findings(result: dict, parsed: ParsedDiff, role: str) -> List[Finding]:
    ...
    for raw in result.get("findings") or []:
        ...
        findings.append(Finding(
            ...
            risk_class=normalize_risk_class(raw.get("risk_class")),
        ))
    return findings
...
class AgenticReviewer(Reviewer):
    ...
    def __init__(
        ...
        # Flywheel trajectory persistence: a callable(record) that stores one
        # content-level trace. It must never raise; role loops guard it anyway.
        self.capture_sink = capture_sink
        # Optional per-role model routing (distill-role / full-replace). A
        # callable(role_name) -> Optional[JsonChatClient]; None falls back to
        # ``self.client``.
        self.role_client_factory = role_client_factory
    ...
    def _review_with_context(
        ...
        # Hermes-style project context: if the repository root carries an
        # EVO_REVIEW.md, surface it as the highest-priority context item.
        memory_context["items"] = list(memory_context.get("items") or [])
        try:
            context_path = os.path.join(root, "EVO_REVIEW.md") if root else ""
            if context_path and os.path.isfile(context_path):
                with open(context_path, "r", encoding="utf-8", errors="replace") as handle:
                    project_context = handle.read(16000).strip()
                if project_context:
                    memory_context["items"].insert(0, {
                        "memory_id": "project-context", "scope": "project",
                        "kind": "project_context",
                        "content": project_context[:4000],
                        "importance": 1.0, "recall_score": 1.0, "created_at": "",
                    })
        except Exception:
            pass
        ledger.trace(
    ...
    def _role_client(self, role: str):
        """Resolve the JsonChatClient for one role, honouring model routing."""
        if self.role_client_factory is None:
            return self.client
        try:
            candidate = self.role_client_factory(role)
        except Exception:
            candidate = None
        return candidate if candidate is not None else self.client

    def _capture_sink_for(self, task_id):
        if self.capture_sink is None:
            return None

        def sink(record):
            item = dict(record)
            item["task_id"] = task_id or ""
            self.capture_sink(item)
    ...
    def _run_lead(
        ...
        role = BoundedRole(
            ...
            ), self._role_client("lead"), self._token_budget("lead"), self.default_time_budget,
            max_steps=max_steps,
            context_manager=self.context_manager,
            working_memory_supplier=working_memory_supplier,
            observation_sink=observation_sink,
        )
        role.trace_sink = self._capture_sink_for(context_key)
    ...
    def _run_critic(
        ...
        role = BoundedRole(
            "critic", CRITIC_PROMPT + (
                ("\nActive validated prompt overlay:\n" + self.prompt_overlay)
                if self.prompt_overlay else ""
            ), self._role_client("critic"), self._token_budget("critic"), self.default_time_budget,
            max_steps=max_steps,
            context_manager=self.context_manager,
            working_memory_supplier=working_memory_supplier,
            observation_sink=observation_sink,
    ...
    def _run_pending_assignments(
        ...
        def run(assignment):
            ...
            role = BoundedRole(
                worker, prompt, self._role_client(worker),
            ...
            role.trace_sink = self._capture_sink_for(task_id)
            context = {
        ...
        for value in values or []:
            try:
                ...
                findings.append(Finding(
                    ...
                    risk_class=normalize_risk_class(value.get("risk_class")),
                ))
            except (TypeError, ValueError):
                continue
        return findings
