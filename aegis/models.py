...
class Finding:
    ...
    # Vertical scenario taxonomy (AI/LLM application security classes). Absent
    # for generic findings so legacy serialized reports are byte-identical.
    risk_class: Optional[str] = None

    def to_dict(self) -> Dict[str, Any]:
        value = asdict(self)
        value["severity"] = self.severity.value
        if value.get("risk_class") is None:
            value.pop("risk_class", None)
        return value
...
_AI_RISK_CLASS_RE = re.compile(r"^[a-z][a-z0-9-]{0,48}$")


def normalize_risk_class(value) -> Optional[str]:
    """Normalize an LLM-supplied risk_class to a safe lowercase token (or None)."""
    raw = str(value or "").strip().lower()
    return raw if _AI_RISK_CLASS_RE.fullmatch(raw) else None
