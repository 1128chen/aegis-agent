...
class VerifiedPatchFixer:
    ...
    def create_fix_commits(
        ...
        branch = "aegis/fix-pr-%d-%s" % (pull_request, stamp)
        commit = client.create_atomic_commit(
            repository, branch, source_sha, changed,
            "fix: apply verified AegisAgent patch for PR #%d" % pull_request,
        )
        draft = client.create_draft_pull_request(
            repository, "fix: verified AegisAgent patch for #%d" % pull_request,
