...
from aegis.evolution import EvolutionEngine, RegressionEvaluator
from aegis.fixer import SafeFixer
...
class AdvancedFeatureTests(unittest.TestCase):
    ...
    def tearDown(self):
        # Windows may briefly hold the SQLite file while a queue/worker thread
        # drains; retry before giving up so cleanup is not flaky.
        import time
    ...
    def test_safe_fixer_changes_only_supported_rules(self):
        ...
        # ast.unparse renders the substitution with either quote style depending on
        # the Python version; both are valid and equivalent.
        self.assertRegex(
            result["content"],
